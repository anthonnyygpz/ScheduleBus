...
export function LiveDashboard() {
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="text-muted-foreground">Error al cargar datos en tiempo real</p>
      </div>
    )
  }
  ...
  const sortedEntries = [...entries].sort((a, b) => {
    const order: Record<string, number> = { active: 0, scheduled: 1, absent: 2, completed: 3 }
    ...
    <div className="flex h-full flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h2 className="text-sm font-semibold text-foreground 2xl:text-base">Jornadas de Hoy</h2>
      ...
      {entries.length === 0 ? (
        <div className="flex flex-1 items-center justify-center rounded-lg border border-border bg-card">
          <p className="text-sm text-muted-foreground">No hay jornadas programadas para hoy</p>
        </div>
      ) : (
          ...
          <Table>
            <TableHeader className="sticky top-0 z-10 bg-card">
              <TableRow className="border-border hover:bg-transparent">
                ...
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs">Empleado</TableHead>
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs">Rol</TableHead>
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs">Turno</TableHead>
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs">
                  <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />Horario</span>
                </TableHead>
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs">
                  <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" />Ruta</span>
                </TableHead>
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs">Progreso</TableHead>
                <TableHead className="text-[11px] font-semibold text-muted-foreground 2xl:text-xs text-right">
                  <span className="inline-flex items-center gap-1"><Route className="h-3 w-3" />Restante</span>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {sortedEntries.map((entry) => (
                ...
                >
                  ...
                  <TableCell className="py-2">
                    <span className="text-xs font-medium text-foreground 2xl:text-sm">{entry.employeeName}</span>
                  </TableCell>
                  <TableCell className="py-2">
                    <span className="text-xs text-muted-foreground 2xl:text-sm">{entry.employeeRole}</span>
                  </TableCell>
                  <TableCell className="py-2">
                    <Badge variant="outline" className={`text-[10px] py-0 px-1.5 2xl:text-xs ${getShiftBadgeClass(entry.shift)}`}>
                  ...
                  <TableCell className="py-2">
                    <span className="text-xs text-muted-foreground 2xl:text-sm">{entry.route}</span>
                  </TableCell>
                  <TableCell className="py-2">
                  ...
                  <TableCell className="py-2 text-right">
                    {entry.status === "active" ? (
                      ...
                      <span className="text-[11px] text-muted-foreground 2xl:text-xs">--</span>
                    ) : entry.status === "absent" ? (
                      <span className="text-[11px] text-destructive 2xl:text-xs">Ausente</span>
                    ) : (
                      <span className="text-[11px] text-muted-foreground 2xl:text-xs">Pendiente</span>
                    )}
                  </TableCell>
                </TableRow>
              ))}
